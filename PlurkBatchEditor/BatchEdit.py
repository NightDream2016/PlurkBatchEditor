...
def requestOauthClientWithUserAuthentication(appKey, appSecret):
    ...
    oauth_token_secret = requestTokenParseResult[b'oauth_token_secret'][0].decode(
        'utf-8')
    ...
    accessTokenResult = get_access_token(
        appKey, appSecret, oauth_token, oauth_token_secret, verifier)
    ...
    oauth_token_secret = accessTokenParseResult[b'oauth_token_secret'][0].decode(
        'utf-8')

    # print('Token:'+ oauth_token + '\nSecret:' + oauth_token_secret)

    client = requestOauthClient(
        appKey, appSecret, oauth_token, oauth_token_secret)
...
def getPlurk(client, id):
    ...
    response = client.request(apiUrl, method='POST',
                              body=resultUrlString.encode('utf-8'))
...
def getPlurks_Multitimes(client, offset, limit, filter):
    ...
    while len(getPlurkList) < int(limit):
        ...
        timeOfLastResult = datetime.datetime.strptime(
            lastResult['posted'], '%a, %d %b %Y %H:%M:%S GMT')
...
def main():
    ...
    while isAuthenticated == False:
        appKeyFile = open('AppKey.txt', 'r', encoding='UTF-8')
        content = appKeyFile.readlines()
        appKey = parse("AppKey = '{}'", content[1].rstrip()).fixed[0]
        appSecret = parse("AppSecret = '{}'", content[2].rstrip()).fixed[0]
        appKeyFile.close()
        ...
        if len(token) != 0:
            client = requestOauthClientFromString(appKey, appSecret, token)
            isAuthenticated = True
            print("Request Auth using local saved Token file...")
        else:
            client = requestOauthClientWithUserAuthentication(
                appKey, appSecret)
            tokenFile.write(client.token.to_string())
            isAuthenticated = True
            print("Request Auth using browser.")
        tokenFile.close()
